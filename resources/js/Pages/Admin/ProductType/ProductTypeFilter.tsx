...
const ProductTypeFilter = ({ onFilter, additionalButtons, statusConst = {} }: ProductTypeFilterProps) => {
  ...
  const getInitialFilters = (): ProductTypeFilters => {
    return {
      name: paramsUrl?.name ?? "",
  ...
  const [filters, setFilters] = useState<ProductTypeFilters>(getInitialFilters());
  ...
  const handleInputChange = (field: keyof ProductTypeFilters, value: string) => {
    setFilters((prev) => ({
      ...prev,
      [field]: value,
    }));
